...
terraform {
  backend "s3" {
    bucket = "antipode"
    key    = "terraform/aws/control_plane/terraform.tfstate"
...
resource "aws_security_group" "control_plane_sg" {
  name        = "${var.vpc_name}-control_plane-sg2"
  description = " Security group for services-vpc control_plane-ami"
  vpc_id      = data.aws_vpc.vpc.id
...
resource "aws_security_group_rule" "docker_daemon" {
  type              = "ingress"
  description       = "control_plane docker daemon ingress"
  from_port         = 2376
  to_port           = 2376
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
...
module "control_plane_base" {
  ...
  security_group_id     = aws_security_group.control_plane_sg.id
  control_plane_ami             = var.control_plane_ami_id
