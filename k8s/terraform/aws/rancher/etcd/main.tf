...
terraform {
  backend "s3" {
    bucket = "antipode"
    key    = "terraform/aws/etcd/terraform.tfstate"
...
resource "aws_security_group" "etcd_sg" {
  name        = "${var.vpc_name}-etcd-sg2"
  description = " Security group for services-vpc etcd-ami"
  vpc_id      = data.aws_vpc.vpc.id
...
resource "aws_security_group_rule" "docker_daemon" {
  type              = "ingress"
  description       = "etcd docker daemon ingress"
  from_port         = 2376
  to_port           = 2376
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
