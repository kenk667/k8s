...
terraform {
  backend "s3" {
    bucket = "antipode"
    key    = "terraform/aws/rancher/terraform.tfstate"
...
resource "aws_security_group" "rancher_sg" {
  name        = "${var.vpc_name}-rancher-sg2"
  description = " Security group for services-vpc rancher-ami"
  vpc_id      = data.aws_vpc.vpc.id
...
resource "aws_security_group_rule" "ssh" {
  ...
  security_group_id = aws_security_group.rancher_sg.id
}
...
resource "aws_security_group_rule" "docker_daemon" {
  type              = "ingress"
  description       = "rancher docker daemon ingress"
  from_port         = 2376
  to_port           = 2376
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
...
module "rancher_base" {
  ...
  security_group_id     = aws_security_group.rancher_sg.id
  rancher_ami               = var.rancher_ami_id
}
