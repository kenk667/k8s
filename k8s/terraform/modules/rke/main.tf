resource "aws_instance" "rke_ec2" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sleep 80 && sudo apt-get update && sudo apt-get upgrade -y && sudo apt-get install -y docker.io rke",
      "cat ~/.ssh/antipode.pub >> ~/.ssh/authorized_keys",
      "sudo mv ~/rke.conf /etc/rke/rke.conf"
    ]
...
resource "aws_eip" "eip" {
  instance = aws_instance.rke_ec2.id
  vpc      = true

  tags = {
    Name = "${var.vpc_name}-rke_ec2-eip"
  }
}
